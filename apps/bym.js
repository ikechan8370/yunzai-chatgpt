...
import { URLSummarizerTool } from '../utils/tools/URLSummarizer.js'
...
export class bym extends plugin {
  ...
  async bym (e) {
    ...
    if (prop < Config.bymRate) {
      ...
      当群友要求你做搜索、URL搜索摘要（也可以进行搜索摘要）、发图、发视频、画图、禁言、踢人、改头衔和昵称等操作时，需要使用工具，不可以直接发文字来蒙混过关。你必须根据需要来调用各种工具，并回复相关问题。要求优先使用中文进行对话。你也可以自行根据当前话题内容选择性的调用这些工具，增加趣味性，以及对违规的群友使用禁言工具进行惩罚，等这种自我主张式的调用方法来进行合理且富有人性的聊天，在进行摘要时需要详细的给出工具摘要后的内容。
以下是你可以使用的表情列表：
